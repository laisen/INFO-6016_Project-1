...
#include <string>
#include "..//INFO-6016_Project 1/cBuffer.h"
#include <sstream>
...
std::string recvMessage(char recvBytes[], int bytesReceived)
{
	...
	//std::cout << "room_length[" << room_length << "]" << std::endl;
	//std::cout << "roomName[" << roomName << "]" << std::endl;
	//std::cout << "client_length[" << client_length << "]" << std::endl;
	//std::cout << "clientName[" << clientName << "]" << std::endl;

	if (message_type == MESSAGE_ID_JOIN_ROOM)
	{
		std::ostringstream outString;
		outString << "Welcome " << clientName << " joins " << roomName << "!";		
	...
	{		
		std::ostringstream outString;
		outString << clientName << " leaves " << roomName << ".";
